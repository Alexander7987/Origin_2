...
public:
	...
	{
		a = other.a;
		string_for_count = other.string_for_count;
		string_for_return = other.string_for_return;
		other.a = "";
		other.string_for_count = "";
		other.string_for_return = "";
	}
...
/* СЛОЖЕНИЕ
	...
	std::string v = "1";
	int b;
	int size_v = std::size(v) - 1; //переменная для корректной работы условного оператора внутри цикла
	int size_a = std::size(a) - 1; //переменная для корректной работы условного оператора внутри цикла
	if (std::size(a) >= std::size(v)) //если левый операнд длиннее чем правый, например 44444 + 98;
	{
		for (i = std::size(a) - 1; i >= 0; i--)
		{
			if (size_v > -1)
				b = (static_cast<int>(a[i]) - static_cast<int>('0')) + (static_cast<int>(v[size_v--]) - static_cast<int>('0'));
			else
				b = static_cast<int>(a[i]) - static_cast<int>('0');
			b += c;
			c = 0;
	...
	if (std::size(v) >= std::size(a)) //если левый операнд длиннее чем правый, например 44444 + 98;
	{
		for (i = std::size(v) - 1; i >= 0; i--)
		{
			if (size_a > -1)
				b = (static_cast<int>(v[i]) - static_cast<int>('0')) + (static_cast<int>(a[size_a--]) - static_cast<int>('0'));
			else
				b = static_cast<int>(v[i]) - static_cast<int>('0');
			b += c;
			c = 0;
			if (b - (b % 10) == 0 && b != 10)
			{
	...
	*/
	...
	std::string string_for_return = "";
	 ...
	 for (i = v - 1; i > 0; i--)
	 {
		 ...
		 string_for_return = "";
		 size_a = std::size(a) - 1;
		 //std::cout << string_for_count << "   ";
	 }


	 /*for (i = 0; i < std::size(string_for_count) / 2; i++)
	 {
		 temp = string_for_count[i];
		 string_for_count[i] = string_for_count[std::size(string_for_count) - 1 - i];
		 string_for_count[std::size(string_for_count) - 1 - i] = temp;
	 } */
